...
class CoCo:
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        print("=> evaluating outdoor CoCo test set...")
        train_dataset = CoCoDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'CoCo', 'train'),
                                        n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        filelist=None,
                                        parse_patches=parse_patches)
        val_dataset = CoCoDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'CoCo', 'val'),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.image_size,
                                      transforms=self.transforms,
                                      filelist=None,
...
class CoCoDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=False):
        super().__init__()
        ...
        if filelist is None:
            ...
            snow_inputs = os.path.join(CoCo_dir, 'input')
            ...
            gt_names += [os.path.join(os.path.join(CoCo_dir, 'gt'), i) for i in images]
